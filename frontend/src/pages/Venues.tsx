...
interface VenueForm { name: string; abbreviation: string; type: 'journal' | 'conference'; url: string; impact_factor: string; ranking: string; deadline: string; location: string }

const EMPTY: VenueForm = { name: '', abbreviation: '', type: 'conference', url: '', impact_factor: '', ranking: '', deadline: '', location: '' };

const RANKINGS = ['A*', 'A', 'B', 'C', 'Q1', 'Q2', 'Q3', 'Q4'];

function Modal({ venue, onClose, onSaved }: { venue?: Venue | null; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<VenueForm>(
    venue ? { name: venue.name, abbreviation: venue.abbreviation || '', type: venue.type, url: venue.url || '', impact_factor: venue.impact_factor ? String(venue.impact_factor) : '', ranking: venue.ranking || '', deadline: venue.deadline || '', location: venue.location || '' }
      : EMPTY
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    const payload = { ...form, impact_factor: form.impact_factor ? parseFloat(form.impact_factor) : undefined };
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h2 className="font-semibold text-slate-900">{venue ? 'Chỉnh sửa venue' : 'Thêm tạp chí / hội nghị'}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400">✕</button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {/* Quick extract */}
          <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-3 space-y-2">
            <p className="text-xs font-medium text-indigo-700 flex items-center gap-1.5"><Sparkles size={12} /> Điền nhanh từ link</p>
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Dán link hội nghị / tạp chí..."
                />
              </div>
          ...
          <div>
            ...
            <input className="input" value={form.name} onChange={set('name')} required placeholder="International Conference on Machine Learning" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label">Tên viết tắt</label>
              <input className="input" value={form.abbreviation} onChange={set('abbreviation')} placeholder="ICML" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {form.type === 'journal' && (
              <div>
                ...
                <input className="input" type="number" step="0.001" value={form.impact_factor} onChange={set('impact_factor')} placeholder="14.255" />
              </div>
            )}
            <div className={form.type === 'journal' ? '' : 'col-span-2'}>
...
export default function Venues() {
  ...
  function VenueTable({ list, emptyText, isConference }: { list: Venue[]; emptyText: string; isConference?: boolean }) {
    if (list.length === 0) return <p className="text-slate-400 text-sm text-center py-6">{emptyText}</p>;
    return (
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              {!isConference && <th className="table-th">Impact Factor</th>}
              {isConference && <th className="table-th">Deadline</th>}
              {isConference && <th className="table-th">Địa điểm</th>}
          ...
          <tbody className="divide-y divide-slate-100">
            {list.map(v => {
              const days = isConference ? daysUntil(v.deadline) : null;
              return (
                <tr key={v.id} className="hover:bg-slate-50">
                  ...
                  {!isConference && <td className="table-td">{v.impact_factor ? <span className="text-green-700 font-medium">{v.impact_factor.toFixed(3)}</span> : <span className="text-slate-400">—</span>}</td>}
                  {isConference && (
                  ...
                  <td className="table-td">
                    <div className="flex gap-1">
                      <button onClick={() => { setEditVenue(v); setShowModal(true); }} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><Edit2 size={13} /></button>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditVenue(null); setShowModal(true); }} className="btn-primary">
          <Plus size={16} /> Thêm hội nghị / Tạp chí
        </button>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div>
            ...
            <VenueTable list={conferences} emptyText="Chưa có hội nghị nào" isConference />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-slate-700 mb-3">Tạp chí ({journals.length})</h3>
            <VenueTable list={journals} emptyText="Chưa có tạp chí nào" />
          </div>
        </>
      )}

      {showModal && (
        <Modal venue={editVenue} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); load(); }} />
      )}
    </div>
  );
}
